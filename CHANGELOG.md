...
- **采样读满时会多消费并丢弃一行**，导致 10 万行只写出 99999 行（已加回归测试）
- **文本列可能因采样之外的长值被截断**：默认改为不限长的 `TEXT`，`--varchar` 时全文件扫描精确取 `VARCHAR(n)`（在 PostgreSQL 上实测修复 `value too long for type character varying(16)`）
